...
class Config:
    ...
    RADIUS_VALUES = [1, 2, 4, 8, 10, 25, 50, 75, 100, 150, 200]  # Test multiple radii
    # RADIUS_VALUES = list(range(1,6))  # Test multiple radii
...
def load_and_clean_fire_data(fire_df: pd.DataFrame) -> pd.DataFrame:
    ...
    fire_df['datetime'] = pd.to_datetime(fire_df['acq_date']) + pd.to_timedelta(
        ...
        fire_df['acq_time'].astype(str).str.zfill(4).str[2:] + ':00')
...
def aggregate_fires_by_zone(fire_df: pd.DataFrame, metadata_df: pd.DataFrame,
    ...
    for zone_idx, zone_row in metadata_df.iterrows():
        ...
        nearby_fires = fire_df[distances <= radius_km].copy()
    ...
    logger.info(f"  Zones with fires: {(fire_by_zone.sum() > 0).sum()}/{len(fire_by_zone.columns)}")
...
def analyze_correlations_for_radius(fire_by_zone: pd.DataFrame, pm25_daily: pd.DataFrame,
...
def run_multi_radius_analysis(fire_df: pd.DataFrame,
                              pm25_df: pd.DataFrame,
                              metadata_df: pd.DataFrame) -> pd.DataFrame:
    """
    Main pipeline for multi-radius fire-PM2.5 correlation analysis
    ...
    """
    logger.info("\n" + "=" * 70)
    logger.info("MULTI-RADIUS FIRE-PM2.5 CORRELATION ANALYSIS")
    logger.info("=" * 70 + "\n")
    ...
    for radius in config.RADIUS_VALUES:
        ...
        fire_by_zone = aggregate_fires_by_zone(fire_clean, metadata_df, radius)

        # Correlation analysis
        radius_results = analyze_correlations_for_radius(fire_by_zone, pm25_daily, radius)
...
def create_grouped_bar_chart(summary_df: pd.DataFrame, output_file: str = 'fire_pm25_grouped_bar.html'):
    """
    ...
    df = summary_df.copy()
    categories = sorted(df['Radius_km'].unique())
    category_labels = [str(int(c)) if float(c).is_integer() else str(c) for c in categories]

    # Pivot so each lag is a column and radii are rows (ensure full category coverage)
    # pivot = df.pivot(index='Radius_km', columns='Lag_Days', values='Avg_Correlation').reindex(categories).fillna(0)
    ...
    fig.update_layout(
        # title={
        #     'text': 'Fire-PM2.5 Correlation Analysis: Impact of Radius and Lag Time',
        #     'x': 0.5,
        #     'xanchor': 'center',
        ...
        xaxis=dict(
            ...
            gridcolor='#ecf0f1',
            title_font={'size': 24, 'color': '#34495e'}
        ),
        yaxis=dict(
        ...
        height=800,
        width=1600,
        font={'family': 'Arial, sans-serif', 'color': '#2c3e50', 'size': 24}
    )
...
if __name__ == "__main__":
    ...
    fire_data = pd.read_csv(
        aq_directory + '/NASA FIRMS DATASET/DL_FIRE_J1V-C2_668887/fire_archive_J1V-C2_668887.csv',
    ...
    # results = run_multi_radius_analysis(fire_data, series, metadata)
    #
    # # Save full results
    # results.to_csv('fire_pm25_multiradius_results.csv', index=False)
    # logger.info("\nFull results saved to: fire_pm25_multiradius_results.csv")
    #
    # # Create summary for visualization
    # summary = create_summary_for_visualization(results)
    # summary.to_csv('fire_pm25_summary.csv', index=False)
    # logger.info("Summary saved to: fire_pm25_summary.csv")

    summary = pd.read_csv('fire_pm25_summary.csv')

    # Create grouped bar chart
    create_grouped_bar_chart(summary, 'fire_pm25_grouped_bar.html')
