...
def prepare_and_save_meteo_data():
...
def one_folder_another(location_main, folders):
    time = pd.to_datetime(folders.split(' to '))
    time = pd.date_range(start=time.min(), end=time.max() + timedelta(days=1), freq='H')[:-1]

    meteoData = np.array(
        [prepare_meteo_data(location_main + '/' + folders + '/' + district + '.csv') for district in meta_data.index])
...
def process_one_folder(location_main, date_folder):
    ...
    meteo_data = np.array(
        [prepare_meteo_data(f'{location_main}/{date_folder}/{district}.csv', district) for district in meta_data.index])
